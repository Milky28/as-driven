...
class WorkbenchApplication:
    ...
    def case_detail(self, issue: int) -> dict[str, Any]:
        ...
        research_result_editor = None
        allowed_actions = set((summary or {}).get("allowed_actions") or [])
        if "import-research" in allowed_actions:
            for key in ("research_result", "research_result_template"):
                ...
                try:
                    ...
                    continue
                if path.is_file():
                    research_result_editor = {
                        "document": _read_json(path),
                        "source": key,
                        "replace": key == "research_result",
                    }
                    break
        return {
            "summary": summary,
            ...
            "research_result_editor": research_result_editor,
        }
...
.research-result-editor{margin:18px 0;padding:16px;border:1px solid #1685aa;background:#0c1b24}.research-result-editor textarea{display:block;width:100%;min-height:520px;margin:10px 0;padding:12px;resize:vertical;color:#d7e5f8;background:#090c10;border:1px solid var(--line);border-radius:5px;font:12px/1.55 Consolas,monospace;tab-size:2}.research-result-editor textarea:focus{outline:2px solid var(--blue);outline-offset:1px}.research-editor-head,.research-editor-actions{display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap}
</style>
</head>
<body>
<main class="shell">
...
function finalizationFailure(message){const marker='release validation failed:';const detail=String(message||'');if(!detail.includes(marker))return `Dataset-release finalization failed: ${detail}`;const failures=detail.slice(detail.indexOf(marker)+marker.length).split(/\s+-\s+/).filter(Boolean);return `Dataset-release validation found ${failures.length} issue${failures.length===1?'':'s'}. This does not undo a promoted contribution or mean it needs more research. Correct the release-wide validation errors, then finalize again.`}
...
const renderDetailWithResearchContext=renderDetail;
renderDetail=function(d){renderDetailWithResearchContext(d);const c=d.case;if(c.submission_type!=='existing-car-research')return;const answers=(c.issue||{}).answers||{},target=c.target_record||{};const heading=document.querySelector('.detail-head h2');if(heading)heading.textContent=`#${d.summary.issue} ${target.display_name||c.issue?.title||''}`;const meta=document.querySelector('.detail-head .case-meta');if(meta)meta.textContent=`Research-only amendment · ${target.record_id||'unresolved record'}`;const first=document.querySelector('.grid .field');if(first)first.innerHTML=`<span>Existing curated record</span>${esc(target.record_id||answers.record||'Not supplied')}`;const approval=document.querySelector('.approval');if(approval&&approval.querySelector('#promoteApproval'))approval.childNodes[1].textContent='I reviewed every record change, retained simulator behavior, driver summary, and source locator.';const body=document.querySelector('.detail-body');const actions=document.querySelector('.actions');if(body&&actions){const context=document.createElement('section');context.className='research-review';context.innerHTML=`<div class="grid"><div class="field"><span>Requested improvement</span>${esc(answers.research_intent||'Not supplied')}</div><div class="field"><span>Exact applicability</span>${esc(answers.applicability||'Not supplied')}</div></div><div class="field"><span>Fields or claims</span>${esc(answers.fields||'Not supplied')}</div><div class="field"><span>Submitted source leads</span>${esc(answers.evidence||'Not supplied')}</div><div class="field"><span>Conflicts or limitations</span>${esc(answers.conflicts||'None supplied')}</div>`;body.insertBefore(context,actions)}}
const renderDetailWithResearchEditor=renderDetail;
renderDetail=function(d){renderDetailWithResearchEditor(d);const state=d.research_result_editor;if(!state)return;const actions=document.querySelector('.actions'),row=document.querySelector('.action-row');if(!actions||!row)return;const file=$('#researchFile');if(file)file.dataset.replace=String(Boolean(state.replace));const button=document.createElement('button');button.className='primary';button.textContent='Edit research result in workbench';const revisit=file?.closest('.revisit');row.insertBefore(button,revisit||row.firstChild);const panel=document.createElement('section');panel.className='research-result-editor';panel.hidden=true;panel.innerHTML=`<div class="research-editor-head"><div><div class="label">Structured research result</div><div class="case-meta">${state.source==='research_result'?'Editing the saved result':'Starting from the generated template'}. Saving validates the document and advances the case when research_status is complete.</div></div><button id="closeResearchEditor" class="ghost">Close editor</button></div><textarea id="researchResultInput" spellcheck="false" aria-label="Research result JSON"></textarea><div class="research-editor-actions"><span class="case-meta">Edit sources, exact locators, field claims, values, confidence, notes, and research_status. Use complete only when the scoped result is ready for final review.</span><div><button id="discardResearchResult" class="ghost">Discard unsaved edits</button> <button id="saveResearchResult" class="primary">Save and validate research</button></div></div>`;actions.parentNode.insertBefore(panel,actions);const editor=$('#researchResultInput'),content=JSON.stringify(state.document,null,2);editor.value=content;editor.dataset.savedValue=content;editor.dataset.replace=String(Boolean(state.replace));const toggle=()=>{panel.hidden=!panel.hidden;button.textContent=panel.hidden?'Edit research result in workbench':'Hide research editor';if(!panel.hidden){editor.focus();panel.scrollIntoView({behavior:'smooth',block:'start'})}};button.onclick=toggle;$('#closeResearchEditor').onclick=toggle;$('#discardResearchResult').onclick=()=>{editor.value=editor.dataset.savedValue;toast('Unsaved research edits discarded')};$('#saveResearchResult').onclick=saveResearchResult}
...
async function saveResearchResult(){const editor=$('#researchResultInput');if(!editor)return;let parsed;try{parsed=JSON.parse(editor.value)}catch(e){return toast(`Research JSON is invalid: ${e.message}`,true)}if(!parsed||Array.isArray(parsed)||typeof parsed!=='object')return toast('Research result must be a JSON object.',true);document.body.classList.add('busy');try{await api(`/api/cases/${selected}/actions/import-research`,{method:'POST',body:JSON.stringify({research_result:parsed,replace:editor.dataset.replace==='true'})});toast('Research result saved and validated');await refresh();await loadArtifact('research_result')}catch(e){toast(e.message,true)}finally{document.body.classList.remove('busy')}}
const runActionWithResearchGuard=runAction;
