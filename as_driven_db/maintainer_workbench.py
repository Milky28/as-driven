...
from .review_feedback import publication_blockers, publish_review_result
...
function beginProgress(button,label,message){document.body.classList.add('busy');document.body.setAttribute('aria-busy','true');document.querySelectorAll('.toolbar button').forEach(item=>item.disabled=true);button.dataset.idleLabel=button.textContent;button.textContent=label;const progress=$('#progress');progress.textContent=message;progress.hidden=false}
function endProgress(button){button.textContent=button.dataset.idleLabel||button.textContent;delete button.dataset.idleLabel;document.querySelectorAll('.toolbar button').forEach(item=>item.disabled=false);$('#progress').hidden=true;document.body.classList.remove('busy');document.body.removeAttribute('aria-busy')}
...
function casesForFilter(cases){const filtered=cases.filter(c=>queueFilter==='all'||queueFilter==='research'&&['identity-research','research-blocked'].includes(c.display_state)||queueFilter==='review'&&['final-review','manifest-review'].includes(c.display_state)||queueFilter==='promoted'&&c.display_state==='promoted'||queueFilter==='published'&&c.publication_status==='published');const rank={'manifest-review':0,'final-review':1,'research-blocked':2,'identity-research':3,'promoted':4,'released':5,'duplicate':6,'published':7};return [...filtered].sort((a,b)=>(rank[a.display_state]??6)-(rank[b.display_state]??6)||(a.issue??0)-(b.issue??0))}
...
$('#refresh').onclick=refreshLocalQueue;$('#sync').onclick=async()=>{document.body.classList.add('busy');try{const r=await api('/api/actions/sync',{method:'POST',body:'{}'});toast(`Sync complete: ${r.processed} processed, ${r.skipped} unchanged`);await refresh(false)}catch(e){toast(e.message,true)}finally{document.body.classList.remove('busy')}};$('#finalize').onclick=async()=>{if(!confirm('Regenerate release outputs, validate, and run the full test suite?'))return;const button=$('#finalize');beginProgress(button,'Finalizing release…','Regenerating release outputs, validating the dataset, and running the full test suite. This can take a few minutes.');try{const r=await api('/api/actions/finalize',{method:'POST',body:JSON.stringify({run_tests:true})});toast(`Dataset ${r.dataset_version} finalized; tests passed`);await refresh()}catch(e){toast(e.message,true)}finally{endProgress(button)}};
