...
Dataset 0.3.33 contains 145 curated records. The identity inventory contains 238
...
- 188 observed identities are covered exactly by curated records.
- 50 observed identities are not covered.
- 38 of those need full guided verification, and they are now the only work
  that requires driving.
- 15 of those 38 have an exact legacy spreadsheet candidate that can seed
  historical controls research; the guided drive must still establish current
  AMS2 behavior and cockpit controls.
- 23 require independent control research in addition to current-game testing.
