...
Dataset 0.3.34 contains 151 curated records. The identity inventory contains 246
...
- 194 observed identities are covered exactly by curated records.
- 52 observed identities are not covered.
- 40 of those need full guided verification, and they are now the only work
  that requires driving.
- 15 of those 40 have an exact legacy spreadsheet candidate that can seed
  historical controls research; the guided drive must still establish current
  AMS2 behavior and cockpit controls.
- 25 require independent control research in addition to current-game testing.
