...
namespace AuthenticControls.Diagnostics
{
        ...
        private static int Main(string[] args)
        {
            if (args.Length < 3)
            {
                Console.Error.WriteLine(
                    "Usage: AuthenticControls.Diagnostics.exe <data/v1> <game> <car identifier>");
                return 2;
            }

            try
            {
                AuthenticControlsDatabase database = AuthenticControlsDatabase.Load(
