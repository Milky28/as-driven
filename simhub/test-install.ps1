...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) (
    "AsDrivenInstallTest-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $sourceLayout = Get-ChildItem -LiteralPath (Join-Path $packageRoot "OverlayLayouts") `
        -File -Filter "As Driven.olayout" | Select-Object -First 1
    if ($null -eq $sourceLayout) {
        throw "The package has no default As Driven layout."
    }
    ...
    $installedPlugin = Join-Path $simHubRoot "AsDriven.Plugin.dll"
    ...
    if (-not (Test-Path -LiteralPath (
        Join-Path $simHubRoot "DashTemplates\As Driven Preflight Overlay"))) {
