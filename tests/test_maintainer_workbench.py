...
class MaintainerWorkbenchTests(unittest.TestCase):
    ...
    def test_page_names_the_review_boundary_and_approval_gates(self) -> None:
        ...
        self.assertIn("Contribution promoted; dataset release pending", page)
        self.assertIn("they do not require more research for this case", page)
        self.assertIn("Finalize dataset release + run tests", page)
        ...
        self.assertIn("Regenerating release-wide outputs, validating the full dataset", page)
        self.assertIn("beginProgress(button,'Finalizing dataset release", page)
        self.assertIn("beginProgress(button,'Syncing...'", page)
        self.assertIn("Complete: dataset ${r.dataset_version} finalized", page)
        self.assertIn("finalizationFailure(e.message)", page)
        self.assertIn("This does not undo a promoted contribution", page)
